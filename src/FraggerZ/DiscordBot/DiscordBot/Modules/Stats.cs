...
public class StatsModule : ModuleBase<SocketCommandContext> {
    ...
    public StatsModule(IUserService userService, IOptions<ChannelSettings> channelSettings, IEmbedService embedService) {
    ...
    public async Task SetSteamURL(string url) {
        ...
        var enteredUri = new Uri(url.Replace("www.", ""));
        ...
        try {
            ...
            await ReplyAsync($"Unable to find your user. run the !register command and try again.");
        }
    }
    ...
    [Command("register")]
    [Summary("Registers the given user or calling user.")]
    public async Task Register(SocketGuildUser user = null) {
        if (Context.Channel.Id != _channelSettings.RoCoStatsChannelId) {
            var message = await ReplyAsync($"Please use this command in <#{_channelSettings.RoCoStatsChannelId}>");
            await message.DeleteAfter(5000);
            return;
        ...
        if (user == null) {
            await _userService.Register(Context.User);
            await Context.Channel.SendMessageAsync($"User {Context.User.Mention} was registered successfully.");
        }
        else {
            await _userService.Register(user);
            await Context.Channel.SendMessageAsync($"User {user.Mention} was registered successfully.");
        }
    }
        ...
        if (Context.Channel is SocketTextChannel channel) {
            ...
            if (region != null) {
                if (region.ToLower() == "na")
                    await Context.Channel.SendMessageAsync(null, false, await _embedService.Leaderboard(gameName, channel));
