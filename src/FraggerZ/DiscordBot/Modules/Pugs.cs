...
public class PugsModule : ModuleBase<SocketCommandContext> {
    ...
    public PugsModule(IRoCoPugService rocoPugService, IUserService userService, IOptions<ChannelSettings> channelSettings
    ...
    public async Task SetupQueue() {
        ...
        string description = $"Users in queue: `{_rocoPugService.NAMainQueue.PlayersInQueue.Count}` / 8\n\n";
        foreach (IUser user in _rocoPugService.NAMainQueue.PlayersInQueue) {
