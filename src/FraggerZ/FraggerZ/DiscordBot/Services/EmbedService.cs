...
namespace DiscordBot.Services {
    public class EmbedService : IEmbedService {
        ...
        public EmbedService(IUserService userService, IOptions<BotSettings> botSettings, IDiscordUserCache discordUserCache,
            IOptions<EmoteSettings> emoteSettings, ITeamService teamService, IOptions<ChannelSettings> channelSettings,
        ...
        public async Task SendTeamScrambleEmbed(Match match, ITextChannel matchTextChannel) {
        ...
        public async Task SendBanEmbed(Match match, IGuildUser user, ITextChannel channel, string rogue) {
        ...
        public async Task SendPlayersNeedReadyUpMessage(Match match, ITextChannel anySTChannel) {
            ...
            foreach (ITextChannel chan in await anySTChannel.Guild.GetTextChannelsAsync()) {
        ...
        public async Task SendAllPlayersReadyEmbed(Match match, ITextChannel anyChannel) {
            // get match channel based on guild text channel query
            ITextChannel channel = null;
            foreach (ITextChannel chan in await anyChannel.Guild.GetTextChannelsAsync()) {
                if (chan.Name.Contains("match") && chan.Name.Contains(match.MatchNumber.ToString())) {
                    channel = chan;
                    break;
                }
        ...
        public async Task SendScrimChallenge(Team team1, Team team2, ITextChannel channel) {
        ...
        public async Task<Embed> Leaderboard(string gameName, ITextChannel channel) {
            List<User> users = await _userService.GetLeaderboard();
            ...
            for (int i = 0; i < users.Count; i++) {
                ...
                IUser discordUser = await channel.GetUserAsync(users[i].DiscordId);
                if (discordUser == null) {
                ...
                count++;
                if (users[i].IGN == null)
                    description += $"`#{count}` - { discordUser.Mention } - **({_userService.GetTier(users[i])})**\n";
                else description += $"`#{count}` - { discordUser.Mention } - `{users[i].IGN}` - **({_userService.GetTier(users[i])})**\n";
        ...
        /// <returns></returns>
        public async Task<Embed> Match(Match match, Team team1 = null, Team team2 = null, List<User> listAllUsers = null) {
            if (match.PlayerDiscordIdsPool.Count > 0)
                return await PickingMatchEmbed(match);
            ...
            foreach (ulong userId in match.Team1DiscordIds) {
                ...
                if (userId == ulong.Parse(match.Team1Ids[0])) {
                    var user = await _discordUserCache.GetUserAsync(ulong.Parse(match.Team1Ids[0]));
                    if (match.GameName == "Rogue Company") description += $"**Host**: {user.Mention} - `{userCurr.IGN}`";
                }
                ...
                if (userCurr.IGN != null) {
                    ...
                    team1Mentions += $"<{_emoteSettings.GamerEmoteUnicode}> " + "**(" + _userService.GetTier(userCurr) + ")** " + ((await _discordUserCache.GetUserAsync(userCurr.DiscordId)).Mention);
            ...
            foreach (ulong userId in match.Team2DiscordIds) {
                ...
                if (userCurr.IGN != null) {
                    ...
                    team2Mentions += $"<{_emoteSettings.GamerEmoteUnicode}> " + "**(" + _userService.GetTier(userCurr) + ")** " + ((await _discordUserCache.GetUserAsync(userCurr.DiscordId)).Mention);
        ...
        public async Task<Embed> PickingMatchEmbed(Match match) {
            string title = $"Match #{match.MatchNumber}";

            //Get captain info
            User captain1 = await _userService.GetById(match.Captain1DiscordId);
            string captain1Mention = (await _discordUserCache.GetUserAsync(captain1.DiscordId)).Mention;
            User captain2 = await _userService.GetById(match.Captain2DiscordId);
            string captain2Mention = (await _discordUserCache.GetUserAsync(captain2.DiscordId)).Mention;
            ...
            foreach (string userId in match.PlayerIdsPool) {
                ...
                string str = $"**({_userService.GetTier(user)})** " + (await _discordUserCache.GetUserAsync(ulong.Parse(userId))).Mention;
            ...
            foreach (ulong userId in match.Team1DiscordIds) {
                ...
                team1Mentions += ($"<{_emoteSettings.GamerEmoteUnicode}> " + (await _discordUserCache.GetUserAsync(userCurr.DiscordId)).Mention + "\n");
            }
            string team2Mentions = "";
            foreach (ulong userId in match.Team2DiscordIds) {
                User userCurr = await _userService.GetById(userId);
                team2Mentions += ($"<{_emoteSettings.GamerEmoteUnicode}> " + (await _discordUserCache.GetUserAsync(userCurr.DiscordId)).Mention + "\n");
            }
            builder.AddField("Team Alpha", team1Mentions);
            builder.AddField("Team Bravo", team2Mentions);
        ...
        public async Task UpdateQueueEmbed(PlayerQueue queue, ITextChannel channel) {
            // Update pug embed in the channel
            var messages = await channel.GetMessagesAsync(1).FlattenAsync();
            IUserMessage message = messages.FirstOrDefault() as IUserMessage;
            if (message == null) {
                return;
            }
